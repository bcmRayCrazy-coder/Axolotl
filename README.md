...
| Arch Linux x64    | pacman 仓库或 `.pkg.tar.zst`          |
| 其他 Linux x64    | `.AppImage`、`.deb` 或 `.rpm`         |

每次正式发布都会同时上传 Tauri 签名的更新包和 `latest.json`。已安装的正式版会从当前仓库的最新 Release 检查更新，并在验证签名后安装。Arch Linux 包由 pacman 管理，因此不会使用应用内更新器覆盖系统文件。

### Arch Linux

先将 Release 中的仓库配置安装到 pacman：

```bash
curl -fsSLo /tmp/axolotl-launcher.repo \
	https://cnb.cool/axlmc/Axolotl/-/releases/latest/download/axolotl-launcher.repo
curl -fsSLo /tmp/axolotl-launcher.gpg \
	https://cnb.cool/axlmc/Axolotl/-/releases/latest/download/axolotl-launcher.gpg
sudo pacman-key --add /tmp/axolotl-launcher.gpg
sudo pacman-key --lsign-key 31BCC0CBEA615E2CA2BFE17DBA3F8314586DA210
sudo install -Dm644 /tmp/axolotl-launcher.repo /etc/pacman.d/axolotl-launcher.conf
sudo grep -qxF 'Include = /etc/pacman.d/axolotl-launcher.conf' /etc/pacman.conf || \
	echo 'Include = /etc/pacman.d/axolotl-launcher.conf' | sudo tee -a /etc/pacman.conf
sudo pacman -Syu axolotl-launcher
```

之后使用 `sudo pacman -Syu` 即可与系统其余软件一起更新 Axolotl Launcher。仓库优先从 CNB 下载，GitHub Release 为自动回退，包与仓库索引均由官方 pacman GPG 密钥签名。预发布仅提供独立 `.pkg.tar.zst`，不会替换稳定 pacman 仓库。也可以从 Release 下载 `.pkg.tar.zst`，使用 `sudo pacman -U <package-file>` 离线安装。安装包仍由 pacman 统一追踪和卸载。
...
1. 将标签版本写入桌面应用构建配置；
2. 在 GitHub 托管的 Windows、macOS 和 Linux runner 上构建安装包，并在 Arch Linux 容器中构建原生 pacman 包；
